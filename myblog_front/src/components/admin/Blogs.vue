    ...
    <el-card class="card1" shadow="never" style="text-align: left">
      ...
      <el-table :data="blogList" border stripe>
                ...
                <el-col class="left-item" :sm="12" :lg="8">
                  <el-card shadow="never">
                    <div class="firstPicture">
                      <h1 style="margin: 5px auto">博客封面 &nbsp;&nbsp;<el-button size="mini" circle type="primary"
        ...
        <el-table-column label="分类" prop="type.name" width="110px">
          <template slot-scope="scope">
            <div @click="changeBlogType(scope.row)" class="change-type">
              <i class="el-icon-edit">{{ scope.row.typeName }}</i></div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取博客列表
    async getBlogList() {
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/blog/getBlogList', {
    ...
    async removeBlogById(id) {
      ...
      const {data: res} = await this.$blog.get('/theAdminOfMyBlogGh/blog/delete/' + id)
    ...
    async changeTypeSubmit() {
      ...
      blog.typeId =  this.editTypeForm.typeId
      // console.log(blog)
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/blog/updateType', {
        blog: blog
      })
      if (res.code === 200) {
        this.editTypeDialogFormVisible = false
    ...
    async handleInputConfirm(row) {
      ...
      // console.log(tagName)
      const res = await this.uploadBlog(row.id, tagName)
      if (res === true) {
        row.inputValue = ''
        row.inputVisible = false
        this.reload()
    ...
    async uploadBlog(inputBlogId, tagName) {
      // 提交数据库，保存修改
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/blog/updateBlogTag', {
        blogId: inputBlogId,
        tagName: tagName
      })
      // console.log(res)
    ...
    async handleClose(i, row) {
      ...
      const res = await this.dealDeletedTag(tag)
      if (res === true) {
        this.reload()
        return this.$message.success('修改博客标签成功！')
      } else {
    ...
    async dealDeletedTag(tag) {
      const {data: res} = await this.$blog.get(`/theAdminOfMyBlogGh/tag/dealDeletedTag/${tag.id}`)
      return res.code === 200
    },
    // 选择类型
    selectType() {
