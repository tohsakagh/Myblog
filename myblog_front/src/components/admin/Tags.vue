            ...
            <el-row :gutter="20">
                <el-col :sm="8" :lg="6" v-for="tag in tagList" style="margin-bottom: 20px;" :key="tag.id">
                    <el-card shadow="hover" style="position:relative;border: 2px solid #eee;text-align: left">
                        ...
                        <p style="margin-bottom: 0">博客数量 <span style="color: #2ac06d">{{tag.blogCount}}</span></p>
                    </el-card>
                </el-col>
            </el-row>
        </el-card>
...
export default {
    ...
    methods: {
        ...
        async deleteTagById(id) {
            ...
            const {data: res} = await this.$blog.get(`/theAdminOfMyBlogGh/tag/delete/${id}`)
            // console.log(res)
            if (res.code === 200) {
        ...
        async getFullTagList() {
            ...
            this.tagList = res.data.sort(this.compare('blogCount'))
        },
