...
export default {
    ...
    methods: {
        ...
        async getViewsData() {
            const {data: res} = await this.$blog.get('/theAdminOfMyBlogGh/blog/getAllViewCount')
            let list = []
        ...
        async getAppreciateCountByMonth() {
            const {data: res} = await this.$blog.get('/theAdminOfMyBlogGh/blog/getAppreciateCount')
            let list = []
            res.data.forEach((item) => {
                let ls = item.split(',')
                list = list.concat({date: ls[0], views: ls[1]})
        ...
        async getCommentCountByMonth() {
            const {data: res} = await this.$blog.get('/theAdminOfMyBlogGh/comment/getCommentCount')
            let list = []
            res.data.forEach((item) => {
                let ls = item.split(',')
                list = list.concat({date: ls[0], views: ls[1]})
