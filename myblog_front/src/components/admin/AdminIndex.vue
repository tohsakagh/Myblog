...
export default {
  ...
  methods: {
    async getcountList() {
      const {data: res} = await this.$blog.get('/theAdminOfMyBlogGh/blog/getBlogCount')
      this.blogCount = res.data
      const {data: res2} = await this.$blog.get('/theAdminOfMyBlogGh/blog/getAllViewCount')
      this.viewCount = res2.data
      const {data: res3} = await this.$blog.get('/theAdminOfMyBlogGh/blog/getAppreciateCount')
      this.appreciateCount = res3.data
      const {data: res4} = await this.$blog.get('/theAdminOfMyBlogGh/comment/getCommentCount')
