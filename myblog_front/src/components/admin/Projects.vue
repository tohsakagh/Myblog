      ...
      <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
        <el-tab-pane label="发布项目" name="first">
          <el-form label-position="left" label-width="80px" style="text-align: left" ref="publishFormRef"
            ...
            <el-form-item label="项目图片">
              <el-upload
                  ref="upload"
                  action="https://ljringh.site/api/tencentcloud/upload/project"
        ...
        <el-tab-pane label="项目列表" name="second">
          <el-table :data="projectList" border stripe>
            ...
            <el-table-column label="项目图片" prop="picUrl">
...
export default {
  ...
  methods: {
    ...
    async publishProject() {
      ...
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/project/saveProject', {
    ...
    async getProjectList() {
      const {data: res} = await this.$blog.get(`/theAdminOfMyBlogGh/project/getProjectList/${this.current}/${this.limit}`)
      if (res.code !== 200) {
    ...
    async removeProjectById(id) {
      ...
      const {data: res} = await this.$blog.get(`/theAdminOfMyBlogGh/project/delete/${id}`)
      if (res.code === 200) {
