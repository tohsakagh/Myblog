      ...
      <ul style="padding: 0;" class="comment-list">
        <li class="comment" v-for="ms in messageList" :key="ms.id">
          ...
          <div class="content">
            ...
            <p>
              <span class="reply">{{ ms.content }}</span>
              <span v-if="administrator" class="delete" @click="deleteMessage(ms.id)">删除</span>
            </p>
          </div>
          <!--                    {{cmt}}-->
        </li>
  ...
  methods: {
    ...
    publish() {
      this.$refs.messageFormRef.validate(async valid => {
        ...
        const {data: res} = await this.$blog.post('/front/message/addMessage', {
    ...
    async deleteMessage(id) {
      console.log(id)
      const {data: res} = await this.$blog.get(`/front/message/delete/${id}`)
      if (res.code === 200) {
        this.getMessageList()
        this.$message({message: "留言删除成功", type: 'success', offset: 80});
      } else {
        this.$message({message: "留言删除失败！", type: 'error', offset: 80});
      }
    },
...
// 获取留言列表
    async getMessageList() {
      const {data: res} = await this.$blog.get(`/front/message/getMessages/${this.current}/${this.limit}`)
