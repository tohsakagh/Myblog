  ...
  <div>
    ...
    <el-dialog style="z-index: 20000" title="上传头像" :visible.sync="dialogVisible">
      <el-upload
              ...
              action="http://localhost/tencentcloud/upload/user"
...
export default {
  ...
  methods: {
    ...
    async setAvatar() {
      ...
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/setAvatar', {
    ...
    async changeUserInfo() {
      const {data: res} = await this.$blog.post('/theAdminOfMyBlogGh/user', {
        user: this.userFrom
      })
      console.log(res)
      if (res.code === 200) {
